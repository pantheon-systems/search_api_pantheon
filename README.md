...
You need to update the schema on the Solr server whenever the generated configuration changes. Running with an outdated schema can cause broken search, indexing errors, and missing features such as highlighting or autocomplete. This includes:
...
# Jump-start config-set from Search API Solr (reset to a known-good baseline)
...
terminus drush <site>.<env> -- search-api-pantheon:postSchema /code/web/private/solr-config/
```
