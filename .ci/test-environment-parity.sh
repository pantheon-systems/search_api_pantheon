#!/bin/bash
# Test environment parity for search_api_pantheon
...
if [ -z "$1" ]; then
  ...
  echo "This script will test all three environments: dev, test, and live"
  exit 1
fi
...
echo "========================================"
echo "Environment Parity Test Suite"
echo "========================================"
log_info "Site: $SITE"
log_info "Testing: dev, test, live environments"
echo "========================================"
...
for ENV in dev test live; do
  ...
  if ! terminus env:info "$SITE.$ENV" >/dev/null 2>&1; then
    log_warning "Environment $ENV not accessible or doesn't exist - skipping"
    continue
  fi
...
if [ -n "${ENV_CORES_FULL[dev]}" ] && [ -n "${ENV_CORES_FULL[test]}" ]; then
  if [ "${ENV_CORES_FULL[dev]}" != "${ENV_CORES_FULL[test]}" ]; then
    ...
    log_error "✗ Dev and Test use same Solr core (they should be different)"
    ((TESTS_FAILED++))
  fi
fi
...
if [ -n "${ENV_CORES_FULL[dev]}" ] && [ -n "${ENV_CORES_FULL[live]}" ]; then
  if [ "${ENV_CORES_FULL[dev]}" != "${ENV_CORES_FULL[live]}" ]; then
    ...
    log_error "✗ Dev and Live use same Solr core (they should be different)"
    ((TESTS_FAILED++))
  fi
fi
...
if [ -n "${ENV_CORES_FULL[test]}" ] && [ -n "${ENV_CORES_FULL[live]}" ]; then
  if [ "${ENV_CORES_FULL[test]}" != "${ENV_CORES_FULL[live]}" ]; then
    ...
    log_error "✗ Test and Live use same Solr core (they should be different)"
    ((TESTS_FAILED++))
  fi
fi
...
UNIQUE_HOSTS=$(printf '%s\n' "${ENV_HOSTS[@]}" | sort -u | wc -l)
UNIQUE_PORTS=$(printf '%s\n' "${ENV_PORTS[@]}" | sort -u | wc -l)

if [ "$UNIQUE_HOSTS" -eq 1 ]; then
  log_success "✓ All environments use same Solr host"
  ((TESTS_PASSED++))
else
  log_error "✗ Environments use different Solr hosts (expected same)"
  ((TESTS_FAILED++))
fi

if [ "$UNIQUE_PORTS" -eq 1 ]; then
  log_success "✓ All environments use same Solr port"
  ((TESTS_PASSED++))
else
  log_error "✗ Environments use different Solr ports (expected same)"
  ((TESTS_FAILED++))
fi

# Test 3: Index content in dev, verify it doesn't appear in test/live
echo ""
...
if [ -n "${ENV_CORES_FULL[dev]}" ] && [ -n "${ENV_CORES_FULL[test]}" ]; then
  ...
  UNIQUE_TITLE="EnvParityTest-$(date +%s)"

  DEV_NODE_ID=$(terminus drush "$SITE.dev" -- ev "
  ...
  if [ -n "$DEV_NODE_ID" ]; then
    log_info "Created node $DEV_NODE_ID in dev with title: $UNIQUE_TITLE"

    # Index in dev
    terminus drush "$SITE.dev" -- search-api:index primary >/dev/null 2>&1 || true
    sleep 3

    # Search for it in dev (should find it)
    DEV_SEARCH=$(terminus drush "$SITE.dev" -- search-api-pantheon:select "title:$UNIQUE_TITLE" --defType="" --rows=1 2>/dev/null | grep -v "notice" | grep -v "WARNING" | jq -r '.response.numFound' || echo "0")

    if [ "$DEV_SEARCH" -gt 0 ]; then
      log_success "✓ Content found in dev environment"
      ((TESTS_PASSED++))
    else
      log_warning "Content not found in dev (might need more time to index)"
    fi
    ...
    if [ -n "${ENV_CORES_FULL[test]}" ]; then
      ...
      if [ "$TEST_SEARCH" -eq 0 ]; then
        log_success "✓ Dev content correctly isolated from test environment"
        ((TESTS_PASSED++))
      else
        log_error "✗ Dev content leaked into test environment (data isolation broken)"
        ((TESTS_FAILED++))
      fi
    fi
  fi
...
if [ $TESTS_FAILED -eq 0 ]; then
  log_success "All environment parity tests passed!"
  exit 0
else
